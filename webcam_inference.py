        ...
        if boxes is not None:
            for box in boxes:
                ...
                if left_eye_img.size != 0:
                    ...
                    utils.draw_bbox_with_label(frame, left_bbox, classes[left_eye_pred.item()])

                if right_eye_img.size != 0:
                    right_eye_img_resized = cv2.resize(right_eye_img, (224, 224))
                    right_eye_tensor = utils.process_image(right_eye_img_resized).unsqueeze(0)
                    ...
                    utils.draw_bbox_with_label(frame, right_bbox, classes[right_eye_pred.item()])
